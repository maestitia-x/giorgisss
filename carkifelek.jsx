...
/* ═══════════════════════════════════════════════════════
   STORAGE & SYNC LAYER
   - Persistent across sessions via window.storage
   - Cross-tab sync via polling every 2s
   ═══════════════════════════════════════════════════════ */
...
function WheelPage({ prizes, participants, updateParticipants, results, updateResults, goPanel }) {
  ...
  // If selected participant was removed (e.g. by another admin), reset
  useEffect(() => {
  ...
  const spin = useCallback(() => {
    ...
    if (part?.assignedPrize) {
      // ASSIGNED PRIZE: wheel MUST land on this specific prize
      winIndex = prizes.findIndex(p => p.name === part.assignedPrize);
      if (winIndex === -1) {
        // Assigned prize no longer exists, fall back to random non-special
      ...
      // NO ASSIGNMENT: only pick from NON-SPECIAL prizes
      const normals = prizes.map((p, i) => ({ ...p, _i: i })).filter(p => !p.special);
      if (!normals.length) {
        // Edge case: all prizes are special — can't spin for unassigned person
        setSpinning(false);
    ...
    /* ─── Calculate exact rotation to land on winIndex ─── */
    // Target: center of slice winIndex must be at top (pointer position)
